...
The browser OPFS schema version 4 gives every lifecycle concept a distinct strict SQLite
...
and is not a `BuildAttempt`. A separate command-receipt table provides strict
idempotency for the shared UI/agent application boundary; receipts cannot change
review, lifecycle, release, or actuation state.
