...
| TM-14 | Browser dependency, worker, renderer, or storage-version drift substitutes code, loses custody, replays stale geometry, collapses lifecycle facts, or upgrades review output into authority (TB-3/TB-4/TB-11). | Exact browser dependency versions and committed pnpm lock; frozen pnpm install in CI; cross-origin-isolated Vite boundary; OPFS-required startup; atomic SQLite `PRAGMA user_version` migration/readback with rejection of newer unsupported schemas; distinct strict lifecycle tables and closed runtime-validated records; current-head-CAS proposal/disposition writes; caller admission of only pre-dispatch build attempts; no caller path for terminal attempt states or evidence closure; channel expected-version CAS; inert approval/export/release custody; immutable revision integrity and transactional stale-head checks; request-scoped Manifold worker gate; review-only mesh disclosure; closed 25-scenario browser behavior corpus; deterministic source-bound 1,000-replay failure-class campaign over 15 predeclared failure scenarios with zero critical counters. | `tests-browser/storage.test.ts`, `tests-browser/worker.test.ts`, `tests-browser/App.test.tsx`, `tests-browser/behavior-corpus.test.ts`, `tests-browser/e2e/golden-path.spec.ts`, and `tests-browser/e2e/behavior-corpus.spec.ts`, including a real version-2 OPFS migration, durable lifecycle-row reopen, closed-corpus execution, and browser campaign verification. | npm registry/publisher, browser, GPU, Vite development server, and same-origin code compromise remain; pnpm lock/frozen install does not prove package code benign or provide signatures/provenance. The campaign is candidate-bound browser evidence, not 1,000 distinct operation schedules, broad browser/OS/GPU qualification, exact-adapter qualification, human review, or G2 acceptance. | Browser/storage/supply-chain owner | Browser dependency/lock/package-manager, Vite headers, OPFS/VFS filename, schema/migration, lifecycle contract/repository API, worker protocol, Manifold/Three.js version, campaign corpus/comparator/source binding, or authority/disclosure changes. |
...
10. The current candidate implements and executes a closed 25-scenario browser behavior corpus and deterministic 1,000-replay browser failure-class campaign over 15 predeclared failure scenarios. It does not execute or claim 1,000 distinct operation schedules. This closes only that candidate-bound browser evidence slice; broader Stage 1/G2 acceptance, browser/OS/GPU/accessibility qualification, exact-adapter coverage, and independent human review remain incomplete.
