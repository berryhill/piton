...
| TM-04 | Hostile source or dependency escapes the precision worker and accesses network, credentials, filesystem, or resources (TB-2/TB-5). | Worker remains `trusted-local` but executes through the repository's Bubblewrap preflight and isolated child boundary; immutable exact request, bounded output custody, resource manifest, no elevated lifecycle authority, and no stronger untrusted-code claim. | `tests/contract/test_worker_contracts.py`, `tests/geometry/test_precision_worker.py`, `tests/test_sandbox_preflight.py`, `scripts/doctor.py`. | Bubblewrap confinement is implemented and verified on the pinned runner, but it does not make hostile or untrusted executable geometry admissible; credentialless operation and broader hostile-code assurance remain incomplete. | Worker/security owner | Any untrusted/generated/imported executable source is admitted, Bubblewrap/preflight policy changes, or claimed isolation class changes. |
...
1. Precision execution uses the implemented Bubblewrap preflight and isolated child boundary but remains classified `trusted-local`; hostile executable geometry is not safely admitted.
2. GitHub-hosted `ubuntu-22.04` is externally administered, and its image contents can change even though the runner label is pinned.
