...
Retry preserves the same task, flow session, worktree, branch, and PR. The final gate is the only step allowed to emit `loop_decision`. Stop rather than retry on secret exposure, ambiguous authority, unsafe fabrication requests, wrong repository/actor, protection bypass, force-push requirement, or corrupt custody. PR publication requires its head/source repository and protected base repository to be the same repository resolved from trusted server-owned task metadata; fork PRs and repository mismatches fail closed as `wrong_repository_or_actor`. Missing merge authorization is a durable wait: preserve the same task/session/PR and resume when server-owned task metadata records either independent GitHub approval, exact trusted operator authorization, or a task-scoped operator grant bound to repository/task/action with `candidate_binding=task_owned_exact_head_after_final_verification`. The final gate mechanically binds a valid task grant to the final verified candidate head.
